...
<script type="text/javascript">
	...
	$(function(){
		$("input[name='checkcode']").blur(function(){
			if(this.value==null || this.value=="") {
				...
				$.post("${pageContext.request.contextPath}/userAction_checkcode",{"inputcode":this.value},function(data){
					if(data){
						$("#checkcodesp").html("<font color='green'>√</font>");
						flag=true;
					}else {
						$("#checkcodesp").html("<font color='red'>×</font>");
						flag=false;
				}
			})
		});	
	...
	function mygo(){
		var formflag=$("#loginform").form("validate");
		if(flag&&formflag) {
			$("#loginform").submit();
		}
	}
				...
				<form id="loginform" name="loginform" method="post" class="niceform"
					...
					<div id="idInputLine" class="loginFormIpt showPlaceholder"
						...
						<input id="loginform:idInput" type="text" name="email"
							class="loginFormTdIpt easyui-validatebox" data-options="required:true,validType:['email','length[0,20]']" maxlength="50" />
						<label for="idInput" class="placeholder" id="idPlaceholder">帐号：</label>
					</div>
					<div class="forgetPwdLine"></div>
					<div id="pwdInputLine" class="loginFormIpt showPlaceholder">
						<input id="loginform:pwdInput" class="loginFormTdIpt easyui-validatebox" data-options="required:true" type="password"
							name="password" value="" />
					...
					<div class="loginFormIpt loginFormIptWiotTh"
						...
						<a href="javascript:void(0);" id="loginform" name="loginform:j_id19" onclick="mygo()">
						<span
							id="loginform:loginBtn" class="btn btn-login"
							style="margin-top:-36px;">登录</span>
						</a>
					</div>
					<span id="checkcodesp"></span>
				</form>
			</div>
		</div>
	</div>
